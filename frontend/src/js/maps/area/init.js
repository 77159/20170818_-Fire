...
$(document).ready(function() {
    ...
    //查看后的弹框点击关闭事件
    $('.right-watch-video-box .right-watch-c').on('click', function() {
        $(this).parent().transition('slide right');
...
function bindBannerItemEvent() {
    ...
    $('.ui.items .item').on('click', function() {
        resetAllMarkersToDefault(); //先清除地图上的高亮的图标
...
//获取窗口模板
function getTbTemplByType(type, index) {
    if (type == 'builds' && index == 0) {
        return getBuildsTableTempl();
    } else if (type == 'warnings') {
        return getWarningTableTempl();
    } else if (type == 'status') {
        return getFireTrTempl();
    } else {
        return getFireResourcesTableTempl();
    }
}

//获取表格模板
function getTrTemplByType(type, index) {
    if (type == 'builds' && index == 0) {
        return getBuildsTrTempl();
    } else if (type == 'warnings') {
        return getWarningTrTempl();
    } else if (type == 'status') {
        return getFireTrTempl();
    } else {
        return getFireResourcesTrTempl();
    }
}
...
function createTableByType(data, type, index) {
    var template = getTbTemplByType(type, index);
    Mustache.parse(template); // optional, speeds up future uses
    var tbHTML = Mustache.render(template, data);

    $('.right-Info-tbl.ui.message').empty().append(tbHTML);
    //bindTableClickEvent();
    bindTableVideoClickEvent();

}

//绑定表格 查看视频 单击事件 start
function bindTableVideoClickEvent() {
    $('.ui.message table tbody tr .right-watch-video').on('click', function() {
        if ($('.right-watch-video-box').hasClass('hidden'))
            $('.right-watch-video-box').transition('slide right');
    });
}
//绑定表格 查看视频 单击事件 end
...
function handleInputEvent(itemsIndex, index, ttype) {
    $('.ui.message input').on('keyup', function() {
        ...
        var template = getTrTemplByType(ttype, index);
        Mustache.parse(template); // optional, speeds up future uses
